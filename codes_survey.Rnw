...
\begin{document}
\title{\large Analysis of survey data on development of virtual learning environment at KWTRP}
\author{JAckline Awino Ochiel\\ 
attachee\\
KWTRP}
...
survey<-read.table("survey.csv",header=T,sep=",")#read in the data
#rename variables
...
names(survey)# view variable names
@
ANALYSIS\\
\large {To get the level of statistical analysis distribution I compared statistical proficiency among the interns and non-interns. I also compared the level of statistical proficiency basing on education level}

<<tidy=TRUE>>=
# creates a new variable group
survey$group[survey$des==1]<-"interns"
survey$group[survey$des!=1]<-"noninterns"
library(ggplot2)#load ggplot2 package to working directory
...
plot2
@
\large{I checked for association between  MOOCS use and the learning prefrence using fishers exact test.}
<<tidy=TRUE>>=
...
@
Rating\\
\large{this shows people views on developing a virtual learning environment within the program rating it as very useful, useful and not useful first bsing on level of education then basing on the role in the program.}
<<tidy=TRUE>>=
